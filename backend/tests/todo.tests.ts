...
describe('Update list name', () => {
    it('should return 200 and updated list when name is valid', async () => {
        const response = await request(app).patch('/lists/1').send({ name: 'Updated List Name' });
        expect(response.status).toBe(200);
        expect(response.body).toEqual({
            id: '1',
    ...
    it('should return 404 for non-existent list', async () => {
        const response = await request(app).patch('/lists/999').send({ name: 'New Name' });
        expect(response.status).toBe(404);
        expect(response.body).toEqual({ message: "No lists with id: 999" });
    });

    it('should return 400 if name is missing', async () => {
        const response = await request(app).patch('/lists/1').send({});
        expect(response.status).toBe(400);
        expect(response.body).toEqual({ message: 'request body must contain name' });
    });

    it('should return 400 if name is not a string', async () => {
        const response = await request(app).patch('/lists/1').send({ name: 123 });
        expect(response.status).toBe(400);
        expect(response.body).toEqual({ 
            message: "invalid name", 
            violations: ["Must be a string"] 
        });
    });

    it('should return 400 if name is empty string', async () => {
        const response = await request(app).patch('/lists/1').send({ name: '' });
        expect(response.status).toBe(400);
        expect(response.body).toEqual({ 
            message: "invalid name", 
            violations: [
    ...
    it('should return 400 if name is too short', async () => {
        const response = await request(app).patch('/lists/1').send({ name: 'Hi' });
        expect(response.status).toBe(400);
        expect(response.body).toEqual({ 
            message: "invalid name", 
            violations: ["Must be at least 3 characters long"]
    ...
    it('should return 400 if name is too long', async () => {
        const longName = 'This is a very long name that exceeds fifty characters limit for validation testing';
        const response = await request(app).patch('/lists/1').send({ name: longName });
        expect(response.status).toBe(400);
        expect(response.body).toEqual({ 
            message: "invalid name", 
            violations: ["Can't be longer than 50 characters"]
    ...
    it('should return 400 if name contains invalid characters', async () => {
        const response = await request(app).patch('/lists/1').send({ name: 'Invalid@Name' });
        expect(response.status).toBe(400);
        expect(response.body).toEqual({ 
            message: "invalid name", 
            violations: ["Only letters, numbers, spaces, and basic punctuation (.,!?:;-_'()) are allowed"]
    ...
    it('should preserve existing items when updating name', async () => {
        // First verify the list has items
        const getResponse = await request(app).get('/lists/1');
        expect(getResponse.body.items).toHaveLength(2);

        // Update the name
        const patchResponse = await request(app).patch('/lists/1').send({ name: 'New Name' });
        expect(patchResponse.status).toBe(200);
        expect(patchResponse.body.title).toBe('New Name');
        expect(patchResponse.body.items).toHaveLength(2);
        expect(patchResponse.body.items).toEqual([
            { id: '1', content: 'Sample item 1', completed: false },
            { id: '2', content: 'Sample item 2', completed: true }
        ]);
    });
